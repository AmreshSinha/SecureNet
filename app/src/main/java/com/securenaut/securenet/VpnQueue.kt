...
object ToNetworkQueueWorker : Runnable {
    ...
    override fun run() {
        ...
        while (!thread.isInterrupted) {
            ...
            if (readCount > 0) {
                ...
                if (packet.isUDP) {
                    ...
                    Log.d(TAG, "Unknown packet protocol type ${packet.ip4Header?.protocolNum}")
                }
            } else if (readCount < 0) {
                break
            }
        ...
        Log.i(TAG, "ToNetworkQueueWorker End of operation")
    }
    ...
    override fun run() {
        while (!thread.isInterrupted) {
            ...
            kotlin.runCatching {
                ...
                Log.e(TAG, "An error occurred while sending udp packet", it)
    ...
    override fun run() {
        while (!thread.isInterrupted) {
            synchronized(udpSocketMap) {
                ...
                if (removeCount > 0) {
                    Log.d(TAG, "Remove $removeCount inactive UDPs that have timed out, currently valid ${udpSocketMap.size}")
                }
            }
            ...
            while (!thread.isInterrupted && iterator.hasNext()) {
                ...
                if (key.isValid) {
                    kotlin.runCatching {
                        ...
                        Log.d(TAG, "An error occurred communicating with the target:${Base64.encodeToString(tcpPipe?.destinationAddress.toString().toByteArray(), Base64.DEFAULT)}")
                        it.printStackTrace()
                        tcpPipe?.closeRst()
                    }
                }
